...
function starterImagePath(starter) {
  return path.join(process.cwd(), 'src', 'assets', 'pokemon', starter.imageFile);
}
...
function buildStarterPayload(selectedKey) {
  ...
  const attachment = new AttachmentBuilder(starterImagePath(starter), { name: starter.imageFile });
  ...
  const embed = new EmbedBuilder()
    ...
    .setImage(`attachment://${starter.imageFile}`)
    .setColor(0x6c5ce7);
...
function buildGuildPayload(starterKey, guildKey) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setThumbnail(`attachment://${starter.imageFile}`)
